...
/*
 * Changes to file:
 * 1. Added optional command line flags "-mm", "-sc", "-lv", "-pv", "-ppv"
 * 2. Added boolean fields:
 *          mainMainVisitor
 *          stringConstantsVisitor
 *          numLocalVarsVisitor
 *          printVisitor
 *          prettyPrintVisitor
 * 3. Added to main method appropriate actions for each flags
 */
...
public class Main {
    ...
    private static void processFlags(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            // if -mm, -sc, -lv, -pv, -ppv then set corresponding boolean to indicate
            // appropriate visitor after the lexer and parser
            else if(args[i].equals("-mm")) {
                mainMainVisitor = true;
            }
            else if(args[i].equals("-sc")) {
                stringConstantsVisior = true;
            }
            else if(args[i].equals("-lv")) {
                numLocalVarsVisitor = true;
            }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (mainMainVisitor) {
                // if mainMainVisitor==true, then print whether
                // Main.main() was found and exit
                MainMainVisitor visitor = new MainMainVisitor();
                System.out.println("Main.main() found: " +
                        visitor.hasMain((Program) result.value));
                System.exit(0);
            }
            if (stringConstantsVisior) {
                // if stringConstantsVisitor==true, then print a list of
                // the String constants and their names and quit
                StringConstantsVisitor visitor = new StringConstantsVisitor();
                System.out.println(
                        visitor.getStringConstants((Program) result.value).toString());
                System.exit(0);
            }
            if (numLocalVarsVisitor) {
                // if numLocalVarsVisitor==true, then print a list of all the method names
                // and their number of local variables and parameters and quit
                NumLocalVarsVisitor visitor = new NumLocalVarsVisitor();
                System.out.println(
                        visitor.getNumLocalVars((Program) result.value).toString());
                System.exit(0);
            }
            if (printVisitor) {
